...
pub fn view<'a>(props: Props<'a>) -> Element<'a, Event> {
    let metrics = ActionBarMetrics {
        title_font_size: props.fonts.ui.size * 0.9,
        ..ActionBarMetrics::default()
    };

    let palette = props.theme.theme.iced_palette();
    let overrides = props.theme.overrides;

    let detail_label = text(props.title).size(metrics.title_font_size);
