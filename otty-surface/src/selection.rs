...
pub enum SelectionType {
    /// Simple point‑to‑point selection.
    Simple,
    /// Column‑wise rectangular block selection.
    Block,
    /// Semantic selection expanded to word or bracket boundaries.
    Semantic,
    /// Line‑wise selection which always covers entire lines.
    Lines,
}
...
impl Selection {
    ...
    pub fn to_range(&self, surface: &Surface) -> Option<SelectionRange> {
        ...
        if end.point.line < surface.topmost_line() {
            return None;
        }
        start.point = start.point.grid_clamp(surface, Boundary::Grid);
        end.point = end.point.grid_clamp(surface, Boundary::Grid);

        match self.ty {
            SelectionType::Simple => self.range_simple(start, end, columns),
            SelectionType::Block => self.range_block(start, end),
            SelectionType::Semantic => {
                Some(Self::range_semantic(surface, start.point, end.point))
            },
            SelectionType::Lines => {
                Some(Self::range_lines(surface, start.point, end.point))
            },
        }
    }
...
mod tests {
    ...
    fn term(height: usize, width: usize) -> Surface {
