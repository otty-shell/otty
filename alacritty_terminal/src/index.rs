...
impl Point {
        ...
        D: Dimensions,
    {
        ...
        let line_changes =
            (rhs + cols - 1).saturating_sub(self.column.0) / cols;
        ...
        D: Dimensions,
    {
        ...
        match boundary {
            ...
            Boundary::Grid if self.line < topmost_line => {
                Point::new(topmost_line, Column(0))
            },
            Boundary::Cursor | Boundary::Grid
                if self.line > bottommost_line =>
            {
...
impl Line {
    ...
    #[must_use]
    pub fn grid_clamp<D: Dimensions>(
        self,
        dimensions: &D,
        boundary: Boundary,
    ) -> Self {
        match boundary {
            Boundary::Cursor => {
                max(Line(0), min(dimensions.bottommost_line(), self))
            },
            Boundary::Grid => {
                let bottommost_line = dimensions.bottommost_line();
                let topmost_line = dimensions.topmost_line();
...
mod tests {
    ...
    fn location_ordering() {
        ...
        assert!(
            Point::new(Line(1), Column(0)) > Point::new(Line(0), Column(0))
        );
        assert!(
            Point::new(Line(0), Column(1)) > Point::new(Line(0), Column(0))
        );
        assert!(
            Point::new(Line(1), Column(1)) > Point::new(Line(0), Column(0))
        );
        assert!(
            Point::new(Line(1), Column(1)) > Point::new(Line(0), Column(1))
        );
        assert!(
            Point::new(Line(1), Column(1)) > Point::new(Line(1), Column(0))
        );
        assert!(
            Point::new(Line(0), Column(0)) > Point::new(Line(-1), Column(0))
        );
    }
