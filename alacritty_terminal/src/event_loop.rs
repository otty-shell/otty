...
const CHILD_TOKEN: Token = Token(1);
...
/// state.
pub struct EventLoop<T: Session + Pollable, U: EventListener> {
    poll: mio::Poll,
    waker: Arc<mio::Waker>,
    pty: T,
    ...
    drain_on_exit: bool,
    ref_test: bool,
    surface: Arc<FairMutex<Surface>>,
}

impl<T, U> EventLoop<T, U>
where
    T: Session + Pollable + 'static,
    U: EventListener + Send + 'static,
{
        ...
        X: Write,
    {
        ...
        // Reserve the next terminal lock for PTY reading.
        let _surface_lease = Some(self.surface.lease());
        let mut surface = None;

        loop {
            // Read from the PTY.
            match self.pty.read(&mut buf[unprocessed..]) {
                // This is received on Windows/macOS when no more data is readable from the PTY.
                Ok(0) if unprocessed == 0 => break,
                Ok(got) => unprocessed += got,
                Err(err) => match err {
                    SessionError::IO(e) => match e.kind() {
                        ...
                        _ => return Err(io::Error::other("oh no")),
                    },
                    _ => return Err(io::Error::other("oh no")),
                },
            }
            ...
            let surface = match &mut surface {
                ...
                None => {
                    surface.insert(match self.surface.try_lock_unfair() {
                        // Force block if we are at the buffer size limit.
                        None if unprocessed >= READ_BUFFER_SIZE => {
                            self.surface.lock_unfair()
                        },
                        None => continue,
                        Some(terminal) => terminal,
                    })
            ...
            state.parser.advance(&buf[..unprocessed], &mut **surface);
    ...
    pub fn spawn(mut self) -> JoinHandle<(Self, State)> {
        thread::spawn_named("PTY reader", move || {
            ...
            if let Err(err) =
                self.pty.register(self.poll.registry(), interest, IO_TOKEN, CHILD_TOKEN)
            {
            ...
            'event_loop: loop {
                ...
                for event in events.iter() {
                    match event.token() {
                        CHILD_TOKEN => {
                            if let Ok(Some(exit_status)) =
                                self.pty.try_get_child_exit_status()
                            {
                        ...
                        IO_TOKEN => {
                            ...
                            if event.is_readable() {
                                if let Err(err) = self.pty_read(
                                    ...
                                    error!(
                                        "Error reading from PTY in event loop: {err}"
                                    );
                                    break 'event_loop;
                                }
                            }
                            ...
                            if event.is_writable() {
                                if let Err(err) = self.pty_write(&mut state) {
                                    error!(
                                        "Error writing to PTY in event loop: {err}"
                                    );
                                    break 'event_loop;
                                }
                            }
                ...
                let want_interest =
                    if needs_write { mio::Interest::READABLE | mio::Interest::WRITABLE }
                    else { mio::Interest::READABLE };

                if want_interest != interest {
                    interest = want_interest;
                    if let Err(err) = self.pty.reregister(self.poll.registry(), interest, IO_TOKEN, CHILD_TOKEN) {
                        error!("reregister failed: {err}");
                        break 'event_loop;
                    }
                }
            }
...
impl EventLoopSender {
    pub fn send(&self, msg: Msg) -> Result<(), EventLoopSendError> {
        ...
        self.waker.wake().map_err(EventLoopSendError::Io)
    }
}
