...
Goal: Give `otty-surface` owned snapshot export that produces `FrameOwned`/`FrameView` for renderers, reusing existing damage tracking.

What to do:
- Introduce `FrameOwned`/`FrameView` types (owned data, `Arc`-cloneable, `'static`) containing size, cursor, mode, display offset, damage information, and visible cells/spans.
...
- Updated surface implementation using `SurfaceModel` and emitting damage into owned frames.
